...
export function Composer({ threadId, project, models, runtimeState, activeTurnId, uncertainTurnStart = false, initialSettings, goal = null, contextUsage, latestCompletedTurnId = null, compact = false, disabled = false, onTextareaReady, onAccessModeChange, onForkLatest, onOpenSideChat }: {
  ...
  useEffect(() => {
    ...
    setRace(false); setResolutionMessage(null); setFeedback(null); setDismissedMenuDraft(null); setCompletedSkillMention(null); setCursor(0);
  ...
  const executeSlashCommand = useCallback(async (raw: string, clientRequestId = newClientRequestId()): Promise<void> => {
    ...
    if (command === "model") {
      ...
      setModel(option.model); setReasoning(option.defaultReasoning); setFeedback({ tone: "success", text: `模型已切换为 ${option.displayName}；将在下一 Turn 生效。` }); return;
    }
  ...
  const resolveUncertainTurn = useMutation({
    ...
    onError: (error) => {
      send.reset(); const code = apiErrorCode(error); if (code === "uncertain_turn_applied") finishSubmission(threadId, true);
  ...
  const queueCommand = useCallback((raw: string) => {
    if (queuedSlashCommand) { setFeedback({ tone: "error", text: `已有排队命令 ${queuedSlashCommand.raw}；请先取消或等待执行。` }); return; }
  ...
  }, [queueSlashCommand, queuedSlashCommand, threadId, updateDraft]);
  ...
  const submit = () => {
    ...
    if (parsed) {
      ...
      if (running) { queueCommand(draft); return; }
    ...
    <div className="composer-shell">
      ...
      <div className={`composer ${running ? "steer-mode" : ""}`}>
        ...
        <div className="composer-toolbar"><div className="access-control"><ShieldCheck size={16} weight={accessMode === "fullAccess" ? "fill" : "regular"} /><select value={accessMode} onChange={(event) => { const next = event.target.value as AccessMode; setAccessMode(next); onAccessModeChange?.(next); persistAccessMode.mutate(next); }} disabled={running || blocked}><option value="fullAccess">Full Access</option><option value="workspaceWrite">Workspace Write</option><option value="readOnly">Read Only</option></select><CaretDown size={11} /></div><span className="toolbar-spacer" />
          ...
          <button className="send-button" onPointerDown={rememberSteerIntent} onClick={submit} disabled={blocked || !draft.trim() || send.isPending || (running && !activeTurnId)} aria-label={running ? "Steer 当前 Turn 或排队 Slash 命令" : "发送或执行命令"}><ArrowUp size={17} weight="bold" /></button>
        </div>
      </div>
    </div>
