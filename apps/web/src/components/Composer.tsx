    ...
    <div className="composer-shell">
      ...
      <div className={`composer ${running ? `${deliveryMode}-mode` : ""}`}>
        ...
        <div className="composer-toolbar">
          <div className="composer-settings" role="group" aria-label="Turn 设置">
            <div className="access-control"><ShieldCheck size={16} weight={accessMode === "fullAccess" ? "fill" : "regular"} /><select aria-label="权限" value={accessMode} onChange={(event) => { const next = event.target.value as AccessMode; setAccessMode(next); onAccessModeChange?.(next); persistAccessMode.mutate(next); }} disabled={running || blocked}><option value="fullAccess">Full Access</option><option value="workspaceWrite">Workspace Write</option><option value="readOnly">Read Only</option></select></div>
            <label className="inline-select model-select"><select aria-label="模型" value={model} onChange={(event) => { const next = event.target.value; setModel(next); setReasoning(preferredReasoningForModel(models.find((item) => item.model === next || item.id === next))); }} disabled={running || blocked}>{models.map((item) => <option key={item.id} value={item.model}>{item.displayName}</option>)}</select></label>
