...
import { api, ApiError, newClientRequestId } from "../api";
import { expectedSteerTurnId } from "../composer-intent";
import { useAppStore } from "../store";

function requestId(): string { return crypto.randomUUID(); }

export function Composer({ threadId, project, models, runtimeState, activeTurnId, initialSettings, compact = false, onTextareaReady, onAccessModeChange }: {
  threadId: string; project: Project; models: ModelOption[]; runtimeState: RuntimeState; activeTurnId?: string;
  initialSettings: { model: string | null; reasoning: string | null; accessMode: AccessMode }; compact?: boolean; onTextareaReady?(element: HTMLTextAreaElement | null): void;
  ...
  }, onSuccess: () => { steerDraftTurnId.current = null; setDraft(threadId, ""); setRace(false); void queryClient.invalidateQueries({ queryKey: ["session", threadId] }); void queryClient.invalidateQueries({ queryKey: ["sessions"] }); }, onError: (error) => { if (error instanceof ApiError && error.status === 409) setRace(true); } });
  ...
  return <div className={`composer-wrap ${compact ? "compact" : ""}`}>{race && <div className="composer-race"><WarningCircle size={16} weight="fill" /><span>当前执行刚刚结束</span><button onClick={() => send.mutate({ forceTurn: true })}>作为下一条消息发送</button><button onClick={() => { steerDraftTurnId.current = null; setRace(false); textarea.current?.focus(); }}>继续编辑</button></div>}
    <div className={`composer ${running ? "steer-mode" : ""}`}>
      <textarea ref={bindTextarea} value={draft} rows={2} onChange={(event) => { rememberSteerIntent(); setDraft(threadId, event.target.value); }} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); rememberSteerIntent(); submit(); } }} placeholder={running ? "向当前执行追加指令" : "要求后续变更"} />
      <div className="composer-toolbar"><div className="access-control"><ShieldCheck size={16} weight={accessMode === "fullAccess" ? "fill" : "regular"} /><select value={accessMode} onChange={(event) => { const next = event.target.value as AccessMode; setAccessMode(next); onAccessModeChange?.(next); }} disabled={running}><option value="fullAccess">Full Access</option><option value="workspaceWrite">Workspace Write</option><option value="readOnly">Read Only</option></select><CaretDown size={11} /></div><span className="toolbar-spacer" />
        ...
        <button className="send-button" onPointerDown={rememberSteerIntent} onClick={submit} disabled={!draft.trim() || send.isPending || (running && !activeTurnId)} aria-label={running ? "Steer 当前 Turn" : "发送"}><ArrowUp size={17} weight="bold" /></button>
      </div>
