    ...
    <div className="composer-shell">
      ...
      <div className={`composer ${running ? `${deliveryMode}-mode` : ""}`}>
        ...
        <div className="composer-toolbar"><div className="access-control"><ShieldCheck size={16} weight={accessMode === "fullAccess" ? "fill" : "regular"} /><select value={accessMode} onChange={(event) => { const next = event.target.value as AccessMode; setAccessMode(next); onAccessModeChange?.(next); persistAccessMode.mutate(next); }} disabled={running || blocked}><option value="fullAccess">Full Access</option><option value="workspaceWrite">Workspace Write</option><option value="readOnly">Read Only</option></select></div><span className="toolbar-spacer" />
          ...
          {running && <button type="button" className={`delivery-mode-toggle ${deliveryMode}`} role="switch" aria-checked={deliveryMode === "queue"} aria-label={`需求发送方式：${deliveryMode === "queue" ? "排队" : "Steer"}`} title={deliveryMode === "queue" ? "当前 Turn 完成后自动发送" : "立即追加到当前 Turn"} onClick={() => { const next = deliveryMode === "steer" ? "queue" : "steer"; setDeliveryMode(next); if (next === "queue") steerDraftTurnId.current = null; setRace(false); }}><span className="delivery-mode-track" aria-hidden="true"><span /></span><span className="delivery-mode-label">{deliveryMode === "queue" ? "排队" : "Steer"}</span></button>}
