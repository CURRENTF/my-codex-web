...
export function PromptSchedule({ threadId }: { threadId: string }) {
  ...
  const save = useMutation({
    mutationFn: (input: Pick<Schedule, "intervalMinutes" | "prompt" | "enabled">) => api<Schedule>(url, { method: "PUT", body: JSON.stringify(input) }),
  ...
  return <Popover.Root open={open} onOpenChange={(next) => {
    setOpen(next);
    if (next) { setMinutes(String(schedule?.intervalMinutes ?? 5)); setPrompt(schedule?.prompt ?? ""); save.reset(); }
  }}>
    ...
    <Popover.Portal><Popover.Content className="schedule-panel" side="top" align="end" sideOffset={12} collisionPadding={14} aria-label="定时轮询设置">
      ...
      {query.isPending ? <p role="status">正在读取设置…</p> : query.isError ? <p role="alert">读取失败：{query.error.message}<button onClick={() => void query.refetch()}>重试</button></p> : <form onSubmit={(event) => { event.preventDefault(); if (valid) save.mutate({ intervalMinutes: Number(minutes), prompt: prompt.trim(), enabled: true }); }}>
        <label className="field-label">轮询间隔<div className="schedule-interval"><input type="number" min="1" max="525600" step="1" required value={minutes} onChange={(event) => setMinutes(event.target.value)} /><span>分钟</span></div></label>
        ...
        <div className="schedule-status" role="status">{schedule?.enabled && schedule.nextRunAt ? <div>下次执行 · {new Date(schedule.nextRunAt).toLocaleString()}</div> : <div>启用后将在一个间隔后首次发送</div>}{schedule?.lastResult && <div>上次检查 · {schedule.lastResult}</div>}<div>任务运行中会跳过本次，不堆积消息。</div></div>
        ...
        <div className="dialog-actions">{schedule?.enabled && <button className="button secondary" type="button" disabled={save.isPending} onClick={() => save.mutate({ intervalMinutes: schedule.intervalMinutes, prompt: schedule.prompt, enabled: false })}>暂停轮询</button>}<button className="button primary" type="submit" disabled={!valid || save.isPending}>{save.isPending ? "保存中…" : schedule?.enabled ? "保存修改" : "启用轮询"}</button></div>
      </form>}
