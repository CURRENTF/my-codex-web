...
function SessionRow({ session, active, projectName, onOpen }: { session: SessionSummary; active: boolean; projectName: string; onOpen(): void }) {
  return <button className={`session-row ${active ? "active" : ""}`} onClick={onOpen}>
    ...
    <span className="session-signals">{session.hasGoal && <Target size={13} weight="bold" />}{session.parentThreadId && <GitFork size={13} weight="bold" />}<StatusIcon state={session.runtimeState} /></span>
  </button>;
}
  ...
  return <aside className="sidebar">
    ...
    <div className="sidebar-list">
      {props.preferences.sidebarMode === "recent" ? <>
        ...
        {props.sessions.map((session) => <SessionRow key={session.threadId} session={session} active={session.threadId === props.activeThreadId} projectName={projectNames[session.projectId] ?? "Other"} onOpen={() => props.onOpen(session.threadId)} />)}
      </> : <>
        ...
        {props.projects.map((project) => {
          ...
          return <section className="project-group" key={project.id} draggable onDragStart={(event) => event.dataTransfer.setData("text/project-id", project.id)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const source = event.dataTransfer.getData("text/project-id"); if (source && source !== project.id) props.onReorder(source, project.id); }}>
            <div className="project-row">
              <button className="project-toggle" onClick={() => setExpanded((state) => ({ ...state, [project.id]: !isExpanded }))}>{isExpanded ? <CaretDown size={14} /> : <CaretRight size={14} />}<span>{project.name}</span></button>
            ...
            {isExpanded && <div className="project-sessions">{visible.map((session) => <SessionRow key={session.threadId} session={session} active={session.threadId === props.activeThreadId} projectName={project.name} onOpen={() => props.onOpen(session.threadId)} />)}
