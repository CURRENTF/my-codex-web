...
export function GoalBar({ threadId, goal, disabled = false }: { threadId: string; goal: Goal | null; disabled?: boolean }) {
  ...
  const save = useMutation({ mutationFn: () => api(`/api/sessions/${threadId}/goal`, { method: "PUT", body: JSON.stringify({ objective: objective.trim(), tokenBudget: budget ? Number(budget) : null, status }) }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["session", threadId] }); setOpen(false); } });
  const clear = useMutation({ mutationFn: () => api(`/api/sessions/${threadId}/goal`, { method: "DELETE" }), onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ["session", threadId] }); setOpen(false); } });
  return <Popover.Root open={open} onOpenChange={setOpen}><Popover.Trigger asChild><button className={`goal-bar ${goal ? "has-goal" : ""}`} disabled={disabled}>
    ...
    <div className="goal-form-row"><label className="field-label">Token Budget<input type="number" min="1" value={budget} onChange={(event) => setBudget(event.target.value)} placeholder="不限" /></label><label className="field-label">Status<select value={status} onChange={(event) => setStatus(event.target.value)}><option value="active">Active</option><option value="paused">Paused</option><option value="blocked">Blocked</option><option value="complete">Complete</option></select></label></div>
